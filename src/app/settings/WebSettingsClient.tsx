...
import { 
  Save, Upload, MapPin, Globe, Layout, 
  Facebook, Twitter, Linkedin, Instagram, Monitor
} from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto pb-20 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between sticky top-0 z-30 theme-bg py-4 border-b theme-border px-4 md:px-0">
        <div>
          ...
          <p className="text-sm text-secondary">Manage theme, footer, map, and social links.</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="space-y-8">
        ...
        {/* --- SECTION 1: VISUALS (Theme & Images) --- */}
          ...
          <div className="flex flex-col md:flex-row gap-8">
             ...
             <div className="w-full md:w-48 space-y-2">
                ...
                <div className="flex items-center gap-3">
                   ...
                   />
                   <span className="text-sm font-mono text-primary uppercase font-bold">{formData.themeColor}</span>
                </div>
             </div>
             ...
             <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <ImageUploadBox 
                  ...
                  helper="Shown when image is missing"
                />
             </div>
          </div>
        </div>

        {/* --- SECTION 2: CONTENT & MAPS --- */}
        <div className="theme-bg theme-border border rounded-xl p-6 space-y-6">
          <h3 className="text-xs font-black text-secondary uppercase tracking-widest border-b theme-border pb-3 mb-4">
             Footer & Location
          </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {/* Right: Map Iframe & Landing Toggle */}
            <div className="space-y-6">
               <div>
               ...
               <div className="flex items-center justify-between p-4 rounded-lg bg-slate-50 dark:bg-white/5 border theme-border">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-secondary">Toggle the main landing page visibility.</p>
                    </div>
                  </div>
        ...
        <div className="theme-bg theme-border border rounded-xl p-6 space-y-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <SocialInput 
                 ...
                 onChange={(v: string) => setFormData({...formData, socialInstagram: v})} 
              />
              <SocialInput 
                 icon={Monitor} label="X (Twitter) Link" 
                 value={formData.socialX} 
                 onChange={(v: string) => setFormData({...formData, socialX: v})} 
              />
              <SocialInput 
                 icon={Facebook} label="Facebook Link" 
                 value={formData.socialFacebook} 
                 onChange={(v: string) => setFormData({...formData, socialFacebook: v})} 
              />
              <SocialInput 
                 icon={Linkedin} label="Linkedin Link" 
                 value={formData.socialLinkedin} 
                 onChange={(v: string) => setFormData({...formData, socialLinkedin: v})} 
              />
              <SocialInput 
                 icon={Globe} label="Pinterest Link" 
                 value={formData.socialPinterest} 
                 onChange={(v: string) => setFormData({...formData, socialPinterest: v})} 
              />
           </div>
        </div>
