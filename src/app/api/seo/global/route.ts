...
const DEFAULT_GLOBAL = {
  ...
  defaults: {
    ...
    twitter: { card: "summary_large_image", fallbackImage: "/og.png" },
  },
  auto: { autoTitlePattern: "{title} | {brand}", autoDescriptionPattern: "{description}", autoSchema: true },
  home: {
...
export async function GET() {
  ...
  try {
    ...
    if (!row) {
      const seeded = await prisma.seoGlobal.create({
        data: { key: KEY, data: DEFAULT_GLOBAL },
      });
      return NextResponse.json({ ok: true, data: seeded.data }, { status: 200 });
    }

    return NextResponse.json({ ok: true, data: row.data }, { status: 200 });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: e?.message || "Failed to load global SEO" }, { status: 500 });
  }
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    // stamp updatedAt inside doc (your UI expects it)
    const nextDoc = {
      ...body,
    ...
    const saved = await prisma.seoGlobal.upsert({
      where: { key: KEY },
      create: { key: KEY, data: nextDoc },
      update: { data: nextDoc },
    });

    return NextResponse.json({ ok: true, data: saved.data }, { status: 200 });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: e?.message || "Failed to save global SEO" }, { status: 500 });
  }
}
