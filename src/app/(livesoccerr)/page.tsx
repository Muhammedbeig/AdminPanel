...
import { firebaseAuth, firestore, firebaseStorage } from "@/lib/firebase/firebase-client";
...
export default function SeoGlobalPage() {
  ...
  const { user, loading, role, roleLoading, refreshClaims } = useAdminAuth();
  ...
  const canEdit = BYPASS || role === "admin" || role === "editor";

  // ✅ FIX: always Record<string,string> (no undefined values)
  async function authHeaders(): Promise<Record<string, string>> {
    if (BYPASS) return {};
    const u = firebaseAuth.currentUser || user;
  ...
  function globalDocRef() {
    return doc(firestore, "sites", siteId, "seo", "global");
  }

  async function seedIfMissing() {
    const ref = globalDocRef();
    const snap = await getDoc(ref);
    if (snap.exists()) return snap.data();

    const seeded = {
      ...deepClone(SEO_STORE_GLOBAL_TEMPLATE),
      updatedAt: new Date().toISOString(),
    };

    await setDoc(ref, seeded, { merge: false });
    return seeded;
  }
  ...
  async function load() {
    ...
    try {
      ...
      // In non-bypass mode, ensure token exists (forces auth ready)
      await authHeaders();

      const loaded = await seedIfMissing();
      setData(loaded);
  ...
  async function save() {
    ...
    try {
      ...
      await setDoc(globalDocRef(), payload, { merge: false });
  ...
  async function uploadImage(file: File, storagePath: string) {
    const ext = (file.name.split(".").pop() || "png").toLowerCase();
    const safeExt = ["png", "jpg", "jpeg", "webp", "svg"].includes(ext) ? ext : "png";
    const fullPath = `${storagePath}/${Date.now()}.${safeExt}`;
  ...
  return (
    <div className="space-y-4">
      ...
      <Section
        title="Brand & Website"
        description="Update the site name, links, and default SEO description shown across the website."
      >
      ...
      {/* Uploads */}
      <Section
        title="Images (Logo & Social Previews)"
        description="Upload images instead of editing file paths. The image URL is saved automatically."
      >
            ...
            <div className="mt-3">
              <input
                ...
                onChange={async (e) => {
                  ...
                  setSaveState({ kind: "saving" });
                  try {
                    const url = await uploadImage(f, `uploads/${siteId}/brand/logo`);
            ...
            <div className="mt-3">
              <input
                ...
                onChange={async (e) => {
                  ...
                  setSaveState({ kind: "saving" });
                  try {
                    const url = await uploadImage(f, `uploads/${siteId}/brand/og`);
            ...
            <div className="mt-3">
              <input
                ...
                onChange={async (e) => {
                  ...
                  setSaveState({ kind: "saving" });
                  try {
                    const url = await uploadImage(f, `uploads/${siteId}/brand/twitter`);
