...
export default function HeaderCommandSearch() {
  ...
  const allCommands = useMemo(() => [
    // --- MAIN (No Developer) ---
    { 
      category: "Main", label: "Dashboard", icon: LayoutDashboard, 
      action: () => router.push("/"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER", "CONTENT_WRITER"] 
    },
    { 
      category: "Main", label: "Media Library", icon: ImageIcon, 
      action: () => router.push("/media"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER", "CONTENT_WRITER"] 
    },
    { 
      category: "Main", label: "Unused Media", icon: ImageMinus, 
      action: () => router.push("/media/unused"), 
      roles: ["ADMIN", "EDITOR"] 
    },

    // --- KNOWLEDGE BASE (No Create for SEO/Editor) ---
    { 
      category: "Knowledge Base", label: "Manage FAQs", icon: HelpCircle, 
      action: () => router.push("/faqs"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "Knowledge Base", label: "Add New FAQ", icon: PlusCircle, 
      action: () => router.push("/faqs/new"), 
      roles: ["ADMIN"] // ✅ Admin only
    }, 
    { 
      category: "Knowledge Base", label: "FAQ Categories", icon: Tag, 
      action: () => router.push("/faqs/categories"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },

    // --- BLOG (No Create for SEO/Editor) ---
    { 
      category: "Blog", label: "All Posts", icon: PenTool, 
      action: () => router.push("/blogs"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER", "CONTENT_WRITER"] 
    },
    { 
      category: "Blog", label: "Write New Post", icon: FileText, 
      action: () => router.push("/blogs/new"), 
      roles: ["ADMIN", "CONTENT_WRITER"] // ✅ Admin & Writer only
    },
    { 
      category: "Blog", label: "Categories", icon: Command, 
      action: () => router.push("/blogs/categories"), 
      roles: ["ADMIN", "EDITOR"] 
    },
    { 
      category: "Blog", label: "Tags", icon: Tags, 
      action: () => router.push("/blogs/tags"), 
      roles: ["ADMIN", "EDITOR"] 
    },
    { 
      category: "Blog", label: "Scheduled Posts", icon: Calendar, 
      action: () => router.push("/blogs/scheduled"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "Blog", label: "Trash", icon: Trash2, 
      action: () => router.push("/blogs/trash"), 
      roles: ["ADMIN", "EDITOR", "CONTENT_WRITER"] 
    },

    // --- SEO MANAGER (No Developer) ---
    { 
      category: "SEO", label: "Global SEO", icon: Globe, 
      action: () => router.push("/seo/global"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "SEO", label: "League SEO", icon: Shield, 
      action: () => router.push("/seo/league"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "SEO", label: "Player SEO", icon: User, 
      action: () => router.push("/seo/player"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "SEO", label: "Redirect Manager", icon: ArrowRightLeft, 
      action: () => router.push("/seo/redirects"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "SEO", label: "Sitemap Manager", icon: FileJson, 
      action: () => router.push("/seo/sitemap"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "SEO", label: "Broken Link Checker", icon: Link2Off, 
      action: () => router.push("/seo/broken-links"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "SEO", label: "Robots.txt Editor", icon: Bot, 
      action: () => router.push("/seo/robots"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },

    // --- SETTINGS (Developer & Admin ONLY) ---
    { 
      category: "Settings", label: "System Settings", icon: Sliders, 
      action: () => router.push("/settings/system"), 
      roles: ["ADMIN", "DEVELOPER"] 
    },
    { 
      category: "Settings", label: "Web Settings", icon: Monitor, 
      action: () => router.push("/settings/web"), 
      roles: ["ADMIN", "DEVELOPER"] 
    },
    { 
      category: "Settings", label: "Profile Settings", icon: Settings, 
    ...
    { 
      category: "Pages", label: "All Custom Pages", icon: Layers, 
      action: () => router.push("/pages"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "Pages", label: "Create New Page", icon: Plus, 
      action: () => router.push("/pages/new"), 
      roles: ["ADMIN"] // ✅ Admin only
    },
    { 
      category: "Pages", label: "Edit Privacy Policy", icon: FileText, 
      action: () => router.push("/seo/pages/privacy"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "Pages", label: "Edit Terms of Service", icon: FileText, 
      action: () => router.push("/seo/pages/terms"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },
    { 
      category: "Pages", label: "Edit Contact Page", icon: FileText, 
      action: () => router.push("/seo/pages/contact"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER"] 
    },

    // --- SYSTEM (All) ---
    { 
      category: "System", label: "Toggle Theme", icon: theme === "dark" ? Sun : Moon, 
      action: () => setTheme(theme === "dark" ? "light" : "dark"), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER", "CONTENT_WRITER", "DEVELOPER"] 
    },
    { 
      category: "System", label: "Logout", icon: LogOut, 
      action: () => logout(), 
      roles: ["ADMIN", "EDITOR", "SEO_MANAGER", "CONTENT_WRITER", "DEVELOPER"] 
    },
  ...
  const getDropdownStyle = () => isDark ? "bg-slate-900 border-slate-700 text-slate-200" : "bg-white border-slate-100 text-slate-800"; 
    ...
    <div ref={containerRef} className="relative w-full max-w-xl mx-auto">
      <div className="relative group z-50">
        ...
        <input
          ref={inputRef}
          type="text"
          className={`block w-full pl-10 pr-12 py-2.5 border leading-5 transition-all duration-200 sm:text-sm font-medium focus:outline-none focus:ring-0 ${getContainerStyle()} ${isOpen ? 'rounded-t-xl rounded-b-none' : 'rounded-xl'}`}
        ...
        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
          <span className={`text-[10px] font-bold border rounded px-1.5 py-0.5 transition-colors duration-200 ${getBadgeStyle()}`}>⌘K</span>
        </div>
      </div>
      ...
      {isOpen && (
        <div className={`absolute top-full left-0 w-full border border-t-0 rounded-b-xl shadow-2xl overflow-hidden z-40 animate-in fade-in zoom-in-95 origin-top ${getDropdownStyle()}`}>
              ...
              filtered.map((cmd, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-[10px] uppercase tracking-wider ${isSelected ? (isDark ? "text-blue-200" : "text-blue-400") : "opacity-50"}`}>{cmd.category}</span>
          ...
          <div className={`px-4 py-2 border-t text-[10px] flex justify-between items-center ${isDark ? "bg-slate-950 border-slate-800 text-slate-500" : "bg-slate-50 border-slate-100 text-slate-400"}`}>
             <div className="flex gap-3"><span><strong>↑↓</strong> navigate</span><span><strong>↵</strong> select</span></div>
